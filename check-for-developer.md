...
- `--check`: 실제 파일을 바꾸지 않고 동기화할 변경 사항만 확인한다. 변경할 내용이 있으면 실패 코드로 종료한다.
...
- 적용 전 source Markdown에서 MDX 표현식, raw HTML/JSX, 지원하지 않는 directive, 잘못된 media frontmatter 등을 검사한다.
